...
| **Jobs & queues** | BullMQ over Redis. Queues: `email`, `discord`, `webhook-retry`, `scheduled`, `marketing` (the `sms` queue was removed with SMS on 2026-09-15). Workers in `src/workers/`, all five running in the Railway worker service via `src/worker-host.ts`. Admin `/admin/queues` reads raw Redis keys (`bull:<name>:wait` etc.) directly — no Bull Board. |
