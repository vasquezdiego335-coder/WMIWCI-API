 ...
 */
function packageLabelOf(jobType?: string | null): string | null {
  const key = (jobType ?? '').trim()
  if (!key || key === 'booking-form') return null
  ...
  try {
    await emailQueue.add('quote-request-received', {
      ...
      payload: {
        ...
        ...(packageLabelOf(lead.jobType) ? { moveSize: packageLabelOf(lead.jobType) } : {}),
...
async function queueInternalAlert(lead: CaptureLead): Promise<{ status: QueueStatus; reason?: string }> {
  ...
  const payload: LeadCardData = {
    ...
    moveDate: lead.moveDate,
    moveSize: packageLabelOf(lead.jobType),
