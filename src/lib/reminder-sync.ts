  ...
  const [bookings, generalExpenses, ownerTxs, leads, customers] = await Promise.all([
    prisma.booking.findMany({
      ...
      include: {
        ...
        expenses: { select: { id: true, itemTitle: true, category: true, amount: true, status: true, receiptUrl: true, vendor: true, createdAt: true } },
      },
      take: 500,
    }),
    prisma.expense.findMany({
      where: { bookingId: null },
      select: { id: true, itemTitle: true, category: true, amount: true, status: true, receiptUrl: true, vendor: true, createdAt: true },
