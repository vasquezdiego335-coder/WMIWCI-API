...
async function processEmailJob(job: Job<EmailJobData>): Promise<void> {
  ...
  //  knows whether the provider actually accepted the message, so it is the
  //  only place allowed to write 'delivered'.
  //
  //  A DEFERRAL IS NOT A FAILURE: quiet hours / caps mean "later", and marking
  //  it failed would make the admin chase a delivery problem that does not
  //  exist. Only a non-deferred refusal is recorded as failed.
