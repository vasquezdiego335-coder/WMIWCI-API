...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }): Promise<NextResponse> {
  ...
  if (!can(session.role as Role, 'money.approve_owner_transaction')) {
    return NextResponse.json({ error: 'Only an owner can approve owner money.' }, { status: 403 })
  }
  ...
  const [updated] = await prisma.$transaction([
    prisma.ownerTransaction.update({ where: { id: params.id }, data: { approvalStatus: parsed.data.approvalStatus } }),
